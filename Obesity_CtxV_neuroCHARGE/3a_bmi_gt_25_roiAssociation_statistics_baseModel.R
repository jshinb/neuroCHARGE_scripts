...
# cohort-specific covariates in the participants BMI >=25 kg/m2.
#
model='baseModel'
BMI_group = "BMI_gt_25"
#*****************************************************************************#
op <- options(warn=1)
cat("\n3a. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the base model in the participants (BMI >=25 kg/m2).\n")

logfile=file.path(
  outdir,
  paste("3a_",BMI_group,"_roiAssociation_statistics_",model,".log",sep='')
...
for(roii in roi_ctx_measurements){
  analdati = data.frame(subset(d %>% filter(BMI>=25),
...
if(file.exists(file.path(outdir,"smooth_plots_adjBase_BMI_gt_25"))){
  unlink(file.path(outdir,"smooth_plots_adjBase_BMI_gt_25"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjBase_BMI_gt_25"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjBase_BMI_gt_25',
                    paste("sm_plot_adjBase_",roi,"%03d_BMI_gt_25.png",sep=""))
...
assoc_res_cov = c()
for(roii in roi_ctx_measurements){
  analdati = subset(analdat%>%filter(BMI>=25),select=c("IID",roii,'logBMI','ICV.scaled','E4_status','age.c','age.c2','sex',
...
adj_ctx_base_BMI_gt_25 = adj_ctx
adj_BMI_base_BMI_gt_25 = adj_BMI
...
#ending message ---------------------------------------------------------------
cat("\nFinished obtaining association estimates for BMI vs. roi-ctx phenotypes under base model (BMI >=25 kg/m2).\n")
options(op)
