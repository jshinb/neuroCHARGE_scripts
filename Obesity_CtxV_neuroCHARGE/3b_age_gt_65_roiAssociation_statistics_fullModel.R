...
#*****************************************************************************#
op <- options(warn=1)
cat("\n3b. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the full model (age >=65 years).\n")

logfile=file.path(
  outdir,
  paste("3b_",age_group,"_roiAssociation_statistics_",model,".log",sep='')
...
# approach1 -------------------------------------------------------------------
adj_ctx = do.call(cbind,lapply(roi_ctx_measurements,get_adj_ctx,d=d%>% filter(age>=65),
...
for(roii in roi_ctx_measurements){
  analdati = data.frame(subset(d%>% filter(age>=65),select = c("IID",std_covariate_names)),
                        adj_BMI,
...
if(file.exists(file.path(outdir,"smooth_plots_adjfull_age_gt_65"))){
  unlink(file.path(outdir,"smooth_plots_adjfull_age_gt_65"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjfull_age_gt_65"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjfull_age_gt_65',paste("sm_plot_adjfull_",roi,"%03d_age_gt_65.png",sep=""))
...
assoc_res_cov = c()
for(roii in roi_ctx_measurements){
  analdati = subset(analdat %>% filter(age >=65),
...
# cleaning-up -----------------------------------------------------------------
adj_ctx_full_age_gt_65 = adj_ctx
adj_BMI_full_age_gt_65 = adj_BMI

save(res1,res2,roi_assoc_res_two_methods,model,age_group,
     file=file.path(outdir,paste("roi_assoc_res_two_methods_",model,"_",age_group,".Rdata",sep='')))

rm(p,res1,res2,res_two_methods,adj_ctxTH,adj_BMI,analdat,analdati,model,age_group)
...
#ending message ---------------------------------------------------------------
cat("\nFinished obtaining association estimates for BMI vs. roi-ctx phenotypes under full model (age >=65 years).\n")
options(op)
