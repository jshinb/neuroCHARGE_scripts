...
# cohort-specific covariates in the participants age>=65 years old.
#
model='baseModel'
age_group = "age_gt_65"
#*****************************************************************************#
op <- options(warn=1)
cat("\n3a. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the base model in the participants (age >=65 years).\n")
...
for(roii in roi_ctx_measurements){
  analdati = data.frame(subset(d %>% filter(age>=65),
                               select = c("FID","IID",std_covariate_names)),
                        adj_BMI,
...
if(file.exists(file.path(outdir,"smooth_plots_adjBase_age_gt_65"))){
  unlink(file.path(outdir,"smooth_plots_adjBase_age_gt_65"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjBase_age_gt_65"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjBase_age_gt_65',
                    paste("sm_plot_adjBase_",roi,"%03d_age_gt_65.png",sep=""))
...
adj_ctx_base_age_gt_65 = adj_ctx
adj_BMI_base_age_gt_65 = adj_BMI
...
#ending message ---------------------------------------------------------------
cat("\nFinished obtaining association estimates for BMI vs. roi-ctx phenotypes under base model (age >=65 years).\n")
options(op)
