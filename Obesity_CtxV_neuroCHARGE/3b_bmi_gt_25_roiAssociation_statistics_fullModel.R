...
if(file.exists(file.path(outdir,"smooth_plots_adjfull_BMI_gt_25"))){
  unlink(file.path(outdir,"smooth_plots_adjfull_BMI_gt_25"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjfull_BMI_gt_25"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjfull_BMI_gt_25',paste("sm_plot_adjfull_",roi,"%03d_BMI_gt_25.png",sep=""))
...
# cleaning-up -----------------------------------------------------------------
adj_ctx_full_BMI_gt_25 = adj_ctx
adj_BMI_full_BMI_gt_25 = adj_BMI

save(
  res1,res2,roi_assoc_res_two_methods,model,BMI_group,
