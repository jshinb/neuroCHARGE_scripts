#*****************************************************************************#
#
# 3a: Examine associations between BMI vs. ctx-phenotypes across the 34 regions
...
#*****************************************************************************#
op <- options(warn=1)
cat("\n3a. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the base model.\n")

logfile=file.path(outdir,"3a_roiAssociation_statistics_baseModel.log"); messages=file(logfile, open="wt")
...
if(file.exists(file.path(outdir,"smooth_plots_adjBase"))){
  unlink(file.path(outdir,"smooth_plots_adjBase"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjBase"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjBase',paste("sm_plot_adjBase_",roi,"%03d.png",sep=""))
...
adj_ctx_base = adj_ctx
adj_BMI_base = adj_BMI
