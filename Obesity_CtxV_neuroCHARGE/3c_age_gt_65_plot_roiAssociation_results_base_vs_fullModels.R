...
cat("\n3c. Loading association estimates for BMI vs. roi-ctxTH for both the base and full models (age >=65 years).\n")
logfile=file.path(outdir,"3c_age_gt_65_plot_roiAssociation_results_base_vs_fullModels.log"); messages=file(logfile, open="wt")
...
for(mod in c('base','full')){
  load(file.path(outdir,paste("roi_assoc_res_two_methods_",mod,"Model_age_gt_65.Rdata",sep='')))
  ...
  png(file.path(outdir,paste('roi_assoc_res_two_methods_',model,"_",age_group,'.png',sep='')),
...
# starting -------------------------------------------------------------------- 
load(file.path(outdir,"roi_assoc_res_two_methods_baseModel_age_gt_65.Rdata"))
res1_base = res1
res2_base = res2 
roi_assoc_res_two_methods_base = roi_assoc_res_two_methods

load(file.path(outdir,"roi_assoc_res_two_methods_fullModel_age_gt_65.Rdata"))
res1_full = res1
res2_full = res2 
...
for(pheno in c('volume','area','thickness')){
  ...
  pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_base_vs_full_Model_age_gt_65.pdf",sep='')),
      width=15,height=8)
  print(p)
  dev.off()
}
