...
infer_APOE = function(data, rs429358_name, rs7412_name){
  ...
  data$E4status <- NA
  data$E4status[!is.na(data$APOE) & data$APOE %in% c('E3E4', 'E4E4')] <- 'e4-carrier'
  data$E4status[!is.na(data$APOE) & data$APOE %in% c('E3E3')] <- 'e4-noncarrier' #no e2
...
write_tsv(d_APOE_wiE4 %>% dplyr::rename(eid=IID) %>% left_join(d_genoPC,by="eid"),file.path(data_dir,"APOE_and_genoPCs_E3_vs_E4.txt"))
