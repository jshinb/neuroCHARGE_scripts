...
get_VolSums <- function(data,ID_col){#data -> cortical volume data 
  ...
  TotalVol = data[['rh_WhiteSurfArea_area']] + data[['lh_WhiteSurfArea_area']]
  SumVol = data.frame(SumVol, TotalVol)
...
get_adj_ctx = function(roii_pheno,d,covariate_names){
  d = d %>% mutate(age.c = scale(age)[,1],
                   age.c2 = (scale(age)/2)^2,
                   ICV = scale(ICV)[,1])# for stability (Gelman)
  analdati = subset(d,select=c("IID","FID",covariate_names,'age.c', 'age.c2',roii_pheno)) 
  ...
  mod0.roii = paste0(c('age.c','age.c2',setdiff(covariate_names,c('age','sex'))),
                     collapse = " + ")
  mod0.roii = paste("y ~ sex*(",mod0.roii,") + (1|FID)",sep="")
  
  fit0.roii = lmer(as.formula(mod0.roii),data=analdati,na.action=na.exclude)
...
get_adj_BMI = function(d,covariate_names){
  analdati = subset(d,select=c('FID','IID','BMI',covariate_names)) %>%
  ...
  mod0 = paste("x ~ sex*(",mod0,") + (1|FID)",sep="")
  
  # age, sex and education
  adj_xname = paste("BMI","adj",sep=".")
  fit0 = lmer(as.formula(mod0),data=analdati,na.action=na.exclude)
