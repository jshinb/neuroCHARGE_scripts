...
cat("\n3c. Loading association estimates for BMI vs. roi-ctx phenotypes for both the base and full models (age <65 years) .\n")
logfile=file.path(outdir,"3c_age_lt_65_plot_roiAssociation_results_base_vs_fullModels.log"); messages=file(logfile, open="wt")
...
for(mod in c('base','full')){
  load(file.path(outdir,paste("roi_assoc_res_two_methods_",mod,"Model_age_lt_65.Rdata",sep='')))
  ...
  png(file.path(outdir,paste('roi_assoc_res_two_methods_',mod,'Model_',age_group,'.png',sep='')),
  ...
  ## forest plots: order by volume in E4-carriers
  require(ggseg)
  ...
  for(pheno in c("volume",'area','thickness')){
    ...
    pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_",model,"_",age_group,".pdf",sep='')),
        width=6,height=6.5)
    print(p + theme_bw())
    dev.off()
    ...
    png(file.path(outdir,paste("brain_image_ctx_",pheno,"_associations_wi_BMI_byApoE_",model,"_",age_group,".png",sep='')),
        height=4, width=6, units="in", res=300)
    print(p_ggseg + theme(panel.spacing = unit(1, "lines")))
    dev.off()
  }
  rm(p,res1,res2,model,age_group)
}
# starting -------------------------------------------------------------------- 
load(file.path(outdir,"roi_assoc_res_two_methods_baseModel_age_lt_65.Rdata"))
res1_base = res1
res2_base = res2 
roi_assoc_res_two_methods_base = roi_assoc_res_two_methods

load(file.path(outdir,"roi_assoc_res_two_methods_fullModel_age_lt_65.Rdata"))
res1_full = res1
res2_full = res2 
...
for(pheno in c('volume','area','thickness')){
  ...
  pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_base_vs_full_Model_age_lt_65.pdf",sep='')),
      width=15,height=8)
  print(p)
  dev.off()
}
...
cat("Finished forest plot of association estimates for BMI vs. roi-ctxTH for both the base and full models (age <65 years).\n")
options(op)
