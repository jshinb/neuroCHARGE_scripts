...
for(filter_i in filter_i0:filter_i1){
  ...
  msg = paste("\n3. Loading association estimates for BMI vs. roi-ctxTH for both the base and full models (",
              group_name,").\n",sep='')
  logfile=file.path(outdir,paste("3_plot_roiAssociation_results_base_vs_fullModels_",filter_group,".log",sep=''))
  messages=file(logfile, open="wt")
  
  cat(msg)
  sink(messages, type="message")
  sink(messages, type="output")
  resfiles = list(base=file.path(outdir,paste("roi_assoc_res_two_methods_base_",filter_group,".Rdata",sep='')),
                  full=file.path(outdir,paste("roi_assoc_res_two_methods_full_",filter_group,".Rdata",sep='')))
  if(all(sapply(resfiles,file.exists))){
    ...
    for(mod in c('base','full')){
      ...
      png(file.path(outdir,paste('roi_assoc_res_two_methods_',model,"_",filter_group,'.png',sep='')),
      ...
      for(pheno in c("volume",'area','thickness')){
        roi_ctx_measurements.ordered = res1 %>% filter(E4_status=="E4-carrier", str_detect(roi,pheno)) %>% 
          arrange(Estimate)
        roi_ctx_measurements.ordered = str_split(roi_ctx_measurements.ordered$roi,"_",simplify = T)[,1]
        res1_pheno = res1 %>% 
          mutate(pheno_roi = str_split(roi,"_",simplify=T)[,2]) %>%
          mutate(roi = str_split(roi,"_",simplify=T)[,1]) %>%
          filter(pheno_roi == pheno,E4_status %in% c("E4-carrier","E4-noncarrier")) %>%
        ...
        pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_",model,"_",filter_group,".pdf",sep='')),
            width=6,height=6.5)
        print(p + theme_bw())
        dev.off()
        ...
        png(file.path(outdir,paste("brain_image_ctx_",pheno,"_associations_wi_BMI_byApoE_",model,"_",filter_group,".png",sep='')),
            height=4, width=6, units="in", res=300)
        print(p_ggseg + theme(panel.spacing = unit(1, "lines")))
        dev.off()
      }
      rm(p,res1,res2,filter_group)
    }#for(mod in c('base','full'))
    ...
    for(pheno in c('volume','area','thickness')){
      ...
      pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_base_vs_full_Model_",filter_group,".pdf",sep='')),
          width=15,height=8)
      print(p)
      dev.off()
    }
    rm(res1,res1_base,res1_full,res1_pheno,res2,res2_base,res2_full,resfiles,
       roi_assoc_res_two_methods_base,roi_assoc_res_two_methods_full,roi_assoc_res_two_methods)
  ...
  msg1=paste("Finished forest plot of association estimates for BMI vs. roi-ctxTH for both the base and full models (",
             group_name,
             ").\n",sep="")
  cat(msg1)
