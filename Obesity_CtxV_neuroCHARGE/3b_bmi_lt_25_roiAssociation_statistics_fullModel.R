...
# adjusting for age, sex, education, ICV (for brain phenotypes), 
# (time between MRI and BMI, if applicable), cohort-specific covariates, and 
# the additional covariates, current smoking, hypertension, and type 2 diabetes 
# status.
#
model='fullModel'
BMI_group = "BMI_lt_25"
#*****************************************************************************#
op <- options(warn=1)
cat("\n3b. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the full model (BMI <25 kg/m2).\n")

logfile=file.path(outdir,paste("3b_",BMI_group,"_roiAssociation_statistics_",model,".log",sep='')); messages=file(logfile, open="wt")
...
# approach1 -------------------------------------------------------------------
adj_ctx = do.call(cbind,lapply(roi_ctx_measurements,get_adj_ctx,d=d%>% filter(BMI <25),
                               covariate_names = cov_ctx))
adj_BMI = get_adj_BMI(d=d%>% filter(BMI <25), covariate_names = cov_BMI)
...
## run fitting full models
for(roii in roi_ctx_measurements){
  analdati = data.frame(subset(d%>% filter(BMI <25),select = c("IID",std_covariate_names)),
                        adj_BMI,
...
if(file.exists(file.path(outdir,"smooth_plots_adjfull_BMI_lt_25"))){
  unlink(file.path(outdir,"smooth_plots_adjfull_BMI_lt_25"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjfull_BMI_lt_25"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  sm_roi = patchwork::wrap_plots(smooth_plot_roi) + 
    ...
    ylab("adjusted for full covariates") 
  
  sm_roi_common_ylim = sm_roi & 
    coord_cartesian(ylim=range(p_ranges_y))
  
  pfile = file.path(outdir,'smooth_plots_adjfull_BMI_lt_25',paste("sm_plot_adjfull_",roi,"%03d_BMI_lt_25.png",sep=""))
...
assoc_res_cov = c()
for(roii in roi_ctx_measurements){
  analdati = subset(analdat %>% filter(BMI <25),
                    select=c("IID",roii,'logBMI','ICV.scaled','E4_status','age.c','age.c2','sex',
...
# cleaning-up -----------------------------------------------------------------
adj_ctx_full_BMI_lt_25 = adj_ctx
adj_BMI_full_BMI_lt_25 = adj_BMI

save(res1,res2,roi_assoc_res_two_methods,model,BMI_group,
     file=file.path(outdir,paste("roi_assoc_res_two_methods_",model,"_",BMI_group,".Rdata",sep='')))

rm(p,res1,res2,res_two_methods,adj_ctxTH,adj_BMI,analdat,analdati,model,BMI_group)
...
#ending message ---------------------------------------------------------------
cat("\nFinished obtaining association estimates for BMI vs. roi-ctx phenotypes under full model (BMI <25 kg/m2).\n")
options(op)
