...
logfile=file.path(outdir,paste("3a_",BMI_group,"_roiAssociation_statistics_",model,".log",sep='')); messages=file(logfile, open="wt")
...
if(file.exists(file.path(outdir,"smooth_plots_adjBase_BMI_lt_25"))){
  unlink(file.path(outdir,"smooth_plots_adjBase_BMI_lt_25"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjBase_BMI_lt_25"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjBase_BMI_lt_25',
                    paste("sm_plot_adjBase_",roi,"%03d_BMI_lt_25.png",sep=""))
