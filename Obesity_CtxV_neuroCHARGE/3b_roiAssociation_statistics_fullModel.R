#*****************************************************************************#
#
# 3b: Examine associations between BMI vs. ctx-phenotypes across the 34 regions
...
#
model='fullModel'
age_group = "age_combined"
#*****************************************************************************#
op <- options(warn=1)
cat("\n3b. Start obtaining association estimates for BMI vs. roi-ctx phenotypes for the full model.\n")

logfile=file.path(outdir,"3b_roiAssociation_statistics_fullModel.log"); messages=file(logfile, open="wt")
...
# approach1 -------------------------------------------------------------------
adj_ctx = do.call(cbind,lapply(roi_ctx_measurements,get_adj_ctx,d=d,
...
for(roii in roi_ctx_measurements){
  analdati = data.frame(subset(d,select = c("IID",std_covariate_names)),
                        adj_BMI,
...
if(file.exists(file.path(outdir,"smooth_plots_adjfull"))){
  unlink(file.path(outdir,"smooth_plots_adjfull"),recursive = T)
}

dir.create(file.path(outdir,"smooth_plots_adjfull"))
for (roi in rois){
  if(roi!="cuneus"){
  ...
  pfile = file.path(outdir,'smooth_plots_adjfull',paste("sm_plot_adjfull_",roi,"%03d.png",sep=""))
...
# cleaning-up -----------------------------------------------------------------
adj_ctx_full = adj_ctx
adj_BMI_full = adj_BMI

save(res1,res2,roi_assoc_res_two_methods,model,age_group,
     file=file.path(outdir,"roi_assoc_res_two_methods_fullModel.Rdata"))
...
#ending message ---------------------------------------------------------------
cat("\nFinished obtaining association estimates for BMI vs. roi-ctx phenotypes under full model.\n")
options(op)
