#*****************************************************************************#
#
# 3c. compare the association results between base vs. full models
#
#*****************************************************************************#
op <- options(warn=1)

cat("\n3c. Loading association estimates for BMI vs. roi-ctxTH for both the base and full models.\n")
logfile=file.path(outdir,"3c_plot_roiAssociation_results_base_vs_fullModels.log"); messages=file(logfile, open="wt")
...
for(mod in c('base','full')){
  load(file.path(outdir,paste("roi_assoc_res_two_methods_",mod,"Model.Rdata",sep='')))
...
for(pheno in c('volume','area','thickness')){
  ...
  pdf(file.path(outdir,paste("forest_plot_assoc_BMI_roi_ctx_",pheno,"_base_vs_full_Model.pdf",sep='')),
      width=15,height=8)
  print(p)
  dev.off()
}
...
cat("Finished forest plot of association estimates for BMI vs. roi-ctxTH for both the base and full models.\n")
options(op)
